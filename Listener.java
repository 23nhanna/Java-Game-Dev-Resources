//Need to add this toJFrame (frame.addKeyListener(keys);)

/*
How to use:

Pass instantiated listener object into whatever class/method needs to use it (the one that was added to the jframe)

For longer keypresses, use a boolean condition and call the keys normal accessor method
For single presses, use the typed accessor method in place of the normal accessor method
If you need to change the state of a key, use the appropriate mutator methods that use booleans as parameters (in progress, i dont feel like doing them right now but if you want you can finish it based off of the only one thats done)
